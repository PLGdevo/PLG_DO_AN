...
public:
    ERaPnP(Transport &_transp, ERaFlash &_flash)
        : Base(_transp, _flash), authToken(nullptr), specSSID(WIFI_SPEC_SSID), specPass(WIFI_SPEC_PASS), hasNetworkCallback(false), scanWiFiConnect(false), persistent(false), scanWiFiRetry(1UL), prevMillisScan(0UL), server(80), udpERa(Udp), newWiFi{}
#if defined(ERA_DETECT_SSL)
          ,
          pClient(nullptr)
#endif
...
void ERaPnP<Transport>::configApi()
{
    ...
    this->server.on("/ota/update", HTTP_POST, [&, this]()
        ...
        this->server.send(200, "text/plane", content); }, [&, this]()
                    {
...
inline void ERaApi<Proto, Flash>::addInfo(cJSON *root)
{
...
template <class Proto, class Flash>
inline void ERaApi<Proto, Flash>::addSelfInfo(cJSON *root)
{
    int16_t signal = WiFi.RSSI();
...
template <class Proto, class Flash>
inline void ERaApi<Proto, Flash>::addModbusInfo(cJSON *root)
{
    int16_t signal = WiFi.RSSI();
